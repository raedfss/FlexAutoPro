...
<?php
// Nota: Asegúrate que este archivo esté guardado como UTF-8 sin BOM
// No debe haber ABSOLUTAMENTE NADA antes de esta etiqueta PHP, ni siquiera espacios en blanco

// Buffer de salida y sesión
ob_start();
session_start();
...
// Comprobación de login (sin redirecciones para evitar problemas con headers)
if (!$is_logged_in) {
    $user_message = '<div class="login-warning">يجب تسجيل الدخول أولاً. <a href="login.php">تسجيل الدخول</a></div>';
}

// Datos del usuario si está logueado
$username = $is_logged_in ? ($_SESSION['username'] ?? 'المستخدم') : '';
$user_type = $is_logged_in ? ($_SESSION['user_role'] ?? 'user') : '';
...
// Conexión a base de datos y procesamiento sólo si está logueado
if ($is_logged_in) {
    ...
    // Registrar búsqueda si hay resultado
...
<head>
    ...
    <title><?= htmlspecialchars($page_title) ?> - FlexAutoPro</title>
    <style>
        /* Estilos globales */
        body {
...
<div class="main-container">
    ...
    // Solo mostrar contenido principal si está logueado
    if ($is_logged_in): 
    ?>
    ...
    <?php endif; // Fin del bloque if ($is_logged_in) ?>
