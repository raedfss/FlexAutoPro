...
try {
    ...
    if (isset($ticket_id) && $ticket_id) {
        // استدعاء ملف send_mail.php لإرسال بريد التنبيه باستخدام PHPMailer
        $emailBody = nl2br($body); // تحويل الأسطر إلى HTML
        sendTicketEmail($primary_email, $username, "🧾 تأكيد استلام التذكرة - $formatted_ticket_id", $emailBody);
